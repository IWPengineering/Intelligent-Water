...
float angleQueue[7];
int prevDay; // Used to trigger the midnightMessage(). Set in initiaization and midnightMessage();
...
* Note: Previously used the internal clock, but uses external now
* TestDate: tbd
********************************************************************/
long timeStamp(void)
{
 ...
 * sends Text Message of all the data collected that day.
 * Note: We are no longer using an interrupt to send a textMessage a the end of the day, instead
 * we are polling.
 * Note2: if any other interrupts need to be added, they go in the "else" section
